...
class HomeController extends Controller
{
	...
	public function summary(Request $request,$booking_id = null,$step = null)
    {
		if(!empty($booking_id))
		{
			...
			$total_charges = $this->calculate_charges($booking,$selected_crew);
			...
			//===============================================================
				...
				{
					$crew_hourly_rate[] = ( $inventory_time + $time_A_B) * ($rate / 60);
					$tot_crew_hourly_rate = $tot_crew_hourly_rate + ( $inventory_time + $time_A_B) * ($rate / 60);
				}
			...
			//===============================================================
				...
				foreach($single_rate as $rate)
				{
					$mob_crew_rate[] = ( $time_H_A + $time_B_H ) * ($rate / 60);
					$tot_mob_crew_rate = $tot_mob_crew_rate + ( $time_H_A + $time_B_H ) * ($rate / 60);
				}
			...
			$mileage_rate_H_A = $dvd_mileage_rate_H_A * $total_mileage_rate;
			$mileage_rate_A_B = $dvd_mileage_rate_A_B * $total_mileage_rate;
			$mileage_rate_B_H = $dvd_mileage_rate_B_H * $total_mileage_rate;
			...
			echo 'total_mileage_rate_A_B : '.($mileage_rate_A_B+$mileage_rate_H_A+$mileage_rate_B_H).'<br>';
			
			 die;
			 
			
			$hourly_time = (($inventory_time/$selected_crew->crew_ratio) + $travel_time) * ($selected_crew->total_rate / 60);			
			$mileage_time = (($inventory_time/$selected_crew->crew_ratio) + $travel_time) * ($booking_form_truck[0]->mileage / 60);			
			
			// $A_B = (($inventory_time/$selected_crew->crew_ratio) + $booking->minutes) * ($booking_form_truck[0]->mileage / 60);
			// $time_hub_A = $booking->time_from_hub * ($booking_form_truck[0]->mileage / 60);
			// $time_hub_B = $booking->time_to_hub * ($booking_form_truck[0]->mileage / 60);
			
			//$mileage_time = $A_B + $time_hub_A + $time_hub_B;
			...
			// echo 'hourly_time : '.$hourly_time.' = (inventory_time: '.($inventory_time/$selected_crew->crew_ratio .'+ minutes: '. $travel_time) .') * (total_rate: '. ($selected_crew->total_rate .' / 60 )').'<br>';
			// echo 'mileage_time : '.$mileage_time.' = (inventory_time: '.($inventory_time/$selected_crew->crew_ratio .'+ minutes: '. $travel_time) .') * (total_rate: '. ($booking_form_truck[0]->mileage .' / 60 )').'<br>';
			
			// echo 'time_A_B : '.$A_B.' = (inventory_time: '.($inventory_time/$selected_crew->crew_ratio .'+ minutes: '. $booking->minutes) .') * (total_rate: '. ($booking_form_truck[0]->mileage .' / 60 )').'<br>';
			// echo 'time_h_A : '.$time_hub_A.' = (minutes: '. $booking->time_from_hub .') * (total_rate: '. ($booking_form_truck[0]->mileage .' / 60 )').'<br>';
			// echo 'time_h_B : '.$time_hub_B.' = (minutes: '. $booking->time_to_hub .') * (total_rate: '. ($booking_form_truck[0]->mileage .' / 60 )').'<br>';
			
			//echo $booking->total_rate;
			...
			{
				$we_pay = $we_pay + $item->we_pay;
				$you_pay = $you_pay + $item->you_pay;
			}
			
			return view('booking.summary',compact('services','booking','booking_location','booking_dates','booking_form_truck','categories','equipments','presets','question','item_images','ranking','items','selected_items','time_charges','calender','insuranceCategories','PropertyInsurance','hourly_time','mileage_time','selected_crew','crew_hourly_rate','tot_crew_hourly_rate','mileage_rate_A_B','tot_mob_crew_rate','mob_crew_rate','mileage_rate_H_A','mileage_rate_B_H','we_pay','you_pay'));
	...
	public function add_new_item($request,$booking_id)
	{
		DB::beginTransaction();
			...
			$item->elevator_time = $request->get('elevator_time');
	...
	public function update_crew($booking_id,$crew)
	{
		$update['crew_count'] = $crew;
		...
		return view('booking.summary.graph',compact('booking','hourly_time','mileage_time','selected_crew','crew_hourly_rate','tot_crew_hourly_rate','mileage_rate_A_B','tot_mob_crew_rate','mob_crew_rate','mileage_rate_H_A','mob_mileage_B_H'));
	...
	public function calculate_charges($booking,$selected_crew)
	{
	...
	//===============================================================
		...
		echo $mileage_rate_A_B.'<br>';
		echo $mileage_rate_H_A.'<br>';
		echo $mileage_rate_B_H.'<br>';
