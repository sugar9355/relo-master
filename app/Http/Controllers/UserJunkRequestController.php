...
class UserJunkRequestController extends Controller
{
    
    public function __construct()
    {
        if (!Helper::authorized('junk_request')){
            return abort(404);
        }
    }
    ...
    public function index()
    {
        ...
        $requests = UserJunkRequest::with('userMovingRequestItems', 'User')->get();
        if ($status == 'ASSIGNED' || $status == 'UNASSIGNED' || $status == 'COMPLETED' || $status == 'Pending' || $status == 'SAVE') {
            $requests = UserJunkRequest::with('userMovingRequestItems', 'User')->where('status', '=', $status)->get();
        }
        return view('admin.junkRequest.index', compact('requests'));
    }
    ...
    {
        $request = UserJunkRequest::with('userMovingRequestItems', 'locations', 'User', 'insuranceDetails')->findOrFail($id);
        return view('admin.junkRequest.show', compact('request'));
    }
    ...
    public function edit($id)
    {
        ...
        $request = UserJunkRequest::with('userMovingRequestItems', 'User', 'locations')->findOrFail($id);
        return view('admin.junkRequest.edit', compact('request', 'zoneTypes', 'vehicleSchedules', 'vehicleTypes'));
    }
    ...
    public function editOrder($id)
    {
        ...
        $items = Inventory::all();
        $request = UserJunkRequest::with('userMovingRequestItems.item.questions.answers', 'User', 'locations')->findOrFail($id);
        $locations = null;
        $locations[] = $request->s_address;
        $locations = array_merge($locations, json_decode($request->waypoints, true));
        if (!is_null($request->d_address)){
            $locations[] = $request->d_address;
        }
        $locations = json_encode($locations);
        return view('admin.junkRequest.edit_order', compact('request', 'serviceTypes', 'items', 'locations'));
    }
    ...
    {
        $userMovingRequests = UserJunkRequest::with('userMovingRequestItems', 'User')
    ...
    public function updateOrder(Request $request, $id)
    {
        ...
        UserJunkRequest::find($id)->update([
            'serviceType' => $serviceType
        ]);*/
        
        UserJunkRequestItem::where('user_moving_request_id', '=', $id)->delete();
        ...
        foreach ($items as $itemIndex => $item) {
            ...
            $newItem = new UserJunkRequestItem($newItem);
            $newItem->save();
        }
        
        return redirect()->route('admin.user_junk_request.index');
    }
    ...
    {
        $status = 'Pending';
        $UserMovingRequest = UserJunkRequest::with('user')->find($id);
        ...
        return redirect()->to(route('admin.user_junk_request.index'));
    }
}
