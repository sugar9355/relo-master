	...
	$(".slider-range").slider({
		...
		slide: function (e, ui) {
			...
			if( (ui.values[0] in start) && (ui.values[1] in end))
			{
				if( start[ui.values[0]] == end[ui.values[1]] )
				{
					$("#end_span_"+e.target.id).html(hours2 + ':' + minutes2 + ' - <font color="red">$' + start[ui.values[0]] + '</font>');
				}
				else
				{
