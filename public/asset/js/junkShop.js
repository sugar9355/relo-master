...
function getQuestions(me, id, lastId = 0){
    ...
    axios.get(`get_questions/${id}/${JSON.stringify(addedToList)}`)
        .then(response => {
            ...
            html += `<div class="row"><div class="col-md-8" style="padding: 15px"><label for="itemPickup" style="margin: 0"> Quantity </label></div><div class="col-md-4"><input placeholder="Quantity" class="form-control my-2" min="1" type="number" id="qty" value="1"></div></div>`;
            html += `<div class="row"><div class="col-md-12"> <label for="itemPickup"> Similar Items </label> <select onchange="changeDialog(this)" onfocus="getLastVal(this)" style="margin: 10px 0 !important; width: 100%;" class="form-control" name="item-pickup">`;
            items.forEach((item) => {
                let selected = "";
                if (item.id == id){
                    selected = "selected";
            ...
            html += `</select></div></div>`;
            html += `<div><img src="${image}" class="img-fluid" alt=""></div>`;
            questions.forEach(question => {
                html += `<div><h1>${question.title}</h1><div><ul class="list-inline">`;
            ...
            html += `<div class="row"><div class="col-md-12"> <label for="itemPickup"> Pickup Location </label> <select style="margin: 10px 0 !important; width: 100%;" class="form-control" name="item-pickup" id="itemPickup">`;
