	...
	<table id="{{date('m', mktime(0, 0, 0, $month, 10))}}" class="table table-bordered text-center">	
		<tr class="bg-warning text-dark">
			...
			@foreach($week as $day) 
				...
				@if($day[0] > 0)
					<button type="button" class="btn btn-link @if(in_array($date,$b_date)) text-white @else text-dark @endif btn-block p-0 position-relative" data-toggle="modal" data-target="#celender{{$day[0]}}">{{$day[0]}}</button>
				@endif
				</td>
