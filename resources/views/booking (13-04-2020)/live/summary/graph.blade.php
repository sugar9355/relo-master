<script>
	...
	<div class="card mt-3" id="insurancebox">
		...
		<div class="card-header bg-dark text-white">
			<h6 class="card-title mb-0"><a data-toggle="collapse" class="text-white" href="#accordion-item-default1"> <h3 class="m-0"><i class="fas fa-file-invoice-dollar fa-large mr-2"></i> Charges <span class="float-right">${{substr($tot_mob_crew_rate+$tot_crew_hourly_rate+$crew_mileage_rate+$mob_mileage_H_A+$mob_mileage_B_H,0,5)}}</span></h3></a></h6>
		</div>
		...
		<div class="card-header pb-0">
			<a data-toggle="collapse" class="text-dark" href="#accordion-item-crew"><p class="font-weight-bold">Crew Charges <span class="float-right">${{substr($tot_crew_hourly_rate+$crew_mileage_rate,0,5)}}</span></p></a>
		</div>
		...
		@endforeach	
			<div class="card-body pb-0">
				<p class="font-weight-bold pb-2">Mileage<span class="float-right">${{substr($crew_mileage_rate,0,4)}}</span></p>
			</div>
		</div>
		...
		<div class="card-header pb-0">
			<a data-toggle="collapse" class="text-dark" href="#accordion-item-mobile"><p class="font-weight-bold">Mobilization Charges <span class="float-right">${{substr($mob_mileage_H_A + $tot_mob_crew_rate + $mob_mileage_B_H,0,4)}}</span></p></a>
		</div>
		<div id="accordion-item-mobile" class="collapse" data-parent="#accordion-mobile">
			<div class="card-body pb-0">
				<p class="font-weight-bold pb-2">Hub to A <span class="float-right">${{substr($mob_mileage_H_A,0,4)}}</span></p>
				
				<p class="font-weight-bold pb-2">A to B <span class="float-right">${{substr($tot_mob_crew_rate,0,5)}}</span></p>
				
				<p class="font-weight-bold pb-2">B to Hub <span class="float-right">${{substr($mob_mileage_B_H,0,5)}}</span></p>
			</div>
		</div>

		</div>	
		
		@include('booking.summary.insurance_charges')
