...
<div class="modal-body">
	...
	<input type="hidden" name="item_image" value="{{ $added_item->item_image }}">
		...
		<div class="form-row">
				...
				<div class="form-group col-md-3 text-center">
					<strong>Quantity</strong> <input class="form-control text-center" min=0 type="number" name="quantity" value="{{ $added_item->quantity }}" disabled >
				</div>
				<div class="form-group col-md-3 text-center">
					@if(isset($categories[0]))
						...
						<option value="{{$category->id}}" @if($added_item->similar_item == $category->id) selected @endif>{{$category->name}}</option>
						@endforeach
						</select>
					@endif
				</div>
		...
		<div class="form-row">
			<div class="form-group col-md-3 ">
				<input class="form-control text-center" value="{{ $added_item->breadth }}" disabled>
			</div> 
			<div class="form-group col-md-3 text-center">
				<input class="form-control text-center" value="{{ $added_item->height }}" disabled>
			</div>
			<div class="form-group col-md-3 text-center">
				<input class="form-control text-center" value="{{ $added_item->width }}" disabled>
			</div>
			<div class="form-group col-md-3 text-center">
				<input class="form-control text-center" value="{{ $added_item->weight }}" disabled>
			</div>
		</div>
		<hr>
			...
			<label class="form-label"><strong>What needs to be disassembled/reassembled?</strong><br>
				...
				@if(!empty($ranking))
					...
					@foreach($ranking as $rank)
							<option value="{{$rank->ranking_id}}" @if($added_item->ranking == $rank->ranking_id) selected @endif>{{$rank->alphabet}} - {{$rank->ranking_name}}</option>
					@endforeach
					</select>
				@endif
				...
				@if(!empty($question))
					...
					@foreach($question as $k => $q)
						...
						@if($q->item_id == $added_item->item_id)	
							...
							<tr>
								<td>
									@if(isset($selected_item_answers[$q->id]->question_id) && $selected_item_answers[$q->id]->question_id == $q->id)
		...
		<div class="form-group">
			...
			<label class="form-label"><strong>Selected Pickup Location of Seleted item</strong></label>
				@if(!empty($booking_location))
					...
					<select name="pick_up_loc_id" class="form-control"  disabled>
					@foreach($booking_location as $loc)
							<option value="{{$loc->booking_loc_id}}" @if($loc->booking_loc_id == $added_item->pick_up_loc_id) selected @endif >{{$loc->location}}</option>
					@endforeach
					</select>
				@endif
			</div>
		...
		<div class="form-group">
			...
			<label class="form-label"><strong>Selected Drop off Location of Seleted item</strong></label>
				@if(!empty($booking_location))
					...
					<select name="drop_off_loc_id" class="form-control" disabled>
					@foreach($booking_location as $loc)
							<option value="{{$loc->booking_loc_id}}" @if($loc->booking_loc_id == $added_item->drop_off_loc_id) selected @endif >{{$loc->location}}</option>
					@endforeach
					</select>
				@endif
			</div>
