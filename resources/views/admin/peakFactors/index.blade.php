...
<div class="card">
		...
		<div class="header-elements">
			<div class="d-flex justify-content-between">
				...
				<div class="list-icons ml-3">
					<form action="{{ route('admin.peakfactor.index') }}" method="POST" enctype="multipart/form-data"
						class="w-100">
							...
							<div class="modal-dialog modal-dialog-centered modal-lg">
								...
								<div class="modal-content">
									<div class="modal-header bg-teal-400 text-white">
										...
										<a href="{{ route('admin.demand_create') }}" class="btn btn-sm btn-warning"
											style="margin-left: 20px;">Add New Demand</a>
									...
									<div class="modal-body">
										...
										{{-- <div class="row">
											<div class="col-md-2 p-2 text-right">
												<span class="badge badge-danger">{{$c->demand_name}}</span>
											</div>
											<div class="col-md-5">
												<input data-id="{{$c->id}}" value="{{$c->min}}" class="form-control min">
											</div>
											<div class="col-md-5">
												<input data-id="{{$c->id}}" value="{{$c->max}}" class="form-control max">
											</div>
										...
										<tr>
											...
											<td><button type="submit" name="customer_demand_remove" value="{{$c->id}}" class="btn btn-sm btn-danger">Remove</button></td>
											<td><a href="{{ route('admin.reservation_fee', ["demand_id" => $c->id])}}" class="btn btn-sm btn-info">Reservation Fee</a></td>
										</tr>
										@endforeach
										</table>
									</div>
	...
	<form action="{{ route('admin.peakfactor.store') }}" method="post" enctype="multipart/form-data" class="w-100">
		{{ csrf_field() }}
		...
		<div class="card-body">
			...
			<div class="row">
				...
				@if(isset($CustomerDemand[0]))
				<div class="col-md-1 p-2 text-right">
					{{-- <span class="badge badge-danger">High Demand - {{$CustomerDemand[0]->high}}</span> --}}
				</div>
				<div class="col-md-1 p-2 text-right">
					{{-- <span class="badge badge-warning">Medrate Demand - {{$CustomerDemand[0]->moderate}}</span> --}}
				</div>
				<div class="col-md-1 p-2 text-right">
					{{-- <span class="badge bg-purple">Low Demand - {{$CustomerDemand[0]->low}}</span> --}}
				</div>
				@endif
				...
				<table class="table table-bordered">
					...
					<tr>
						...
						<td>{{ ($day ? $day : '&nbsp;') }}
							...
							<br>
							@if($day > 0)
							<input name="peak[{{$c_date['now_month']}}][{{$day}}]" value="{{ $peakFactor[$day] or '' }}"
