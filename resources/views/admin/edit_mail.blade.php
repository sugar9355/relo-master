...
<option value="other" {{$template[0]->template_id!="0" && $template[0]->template_id!="100" && $template[0]->template_id>"15" ?"selected":""}}>Other</option>
  ...
  <div class="form-group">
    ...
    <input type="text" required class="form-control" id="fromemail" value="{{$template[0]->from_email}}" name="fromemail" placeholder="Email">
  </div>
          ...
          if (form.checkValidity() === false) {
             ...
             var tempidd = $('#templateid').val()!='other'?$('#templateid').val():$('#otheroption').val()
                $.ajax({
                  ...
                  data: {'tid':$('#tid').val(),'temp_id':tempidd,'fromemail':$('#fromemail').val(),'subject':$('#subject').val(),'template_value':CKEDITOR.instances.mailtemplate.getData()},
                ...
                {
                 location.href = "{{url('admin/canned_mail')}}"
                }
                });
                  }
                })
