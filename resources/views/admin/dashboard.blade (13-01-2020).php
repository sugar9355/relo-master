		  ...
		  <div class="panel-group">
			<div class="panel panel-default">
				...
				<div class="panel-heading">
					<div style="float:left;"><h5><strong>{{date("F")}} ({{date("Y")}})</strong></h5></div>
			  ...
			  <div class="panel-body">
				<table class="table table-bordered">
					...
					@foreach ($calender[intval(date('m'))] as $k =>$week) 
						<tr>
						@foreach($week as $day) 
							<td width="14%" height="80px">{{ ($day ? $day : '&nbsp;') }}<br>
							@foreach($jobs as $job) 
							
								@if(isset($job->day) && $job->day == $day)
									...
									<button type="button" class="btn {{$btn_color}} btn-sm col-md-12 c_btn">{{$job->primary_date}} - {{$job->start_time}} 
									@if($job->truck_id == '')
