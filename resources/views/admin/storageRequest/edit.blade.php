    ...
    <div class="card">
        <div class="card-body">
            <div >
                ...
                <div class="row">
                    <form action="{{ route('admin.user_storage_request.update', $request->id) }}" method="post">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="col-md-12">
                                                        <a target="_blank" class="btn btn-block btn-info"
                                                           href="{{ route('admin.user_storage_request.show', $request->id) }}">View
                                                            Inventory</a>
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <div id="collapseThree" class="panel-collapse collapse" role="tabpanel"
                                        ...
                                        <div class="panel-body">
                                            ...
                                            <div class="col-md-5">
                                                <div class="form-group">
                                                    <div class="col-md-6">
                                                        @if($request->drop_date_type == "FF")
                                                            <div class="form-group">
                                                                <div class="col-md-12">
                                                                    <label for="mySelect">Select Drop Date</label>
                                                                </div>
                                                                <div class="col-md-12">
                                                                    <select name="drop_booking_date" class="form-control" onchange="changeTime(this, 'dropTime')"
                                                                            id="mySelect">
                                                                        <optgroup label="Preferred Drop Date">
                                                                            <option value="{{ $request->drop_prefer_date }}"
                                                                                    data-time="{{ $request->prefer_time }}">{{ $request->drop_prefer_date }}</option>
                                                                        </optgroup>
                                                                        <optgroup label="Secondary Drop Date">
                                                                            @foreach(explode(',', $request->drop_date) as $index => $date)
                                                                                <option value="{{ $date }}"
                                                                                        data-time='{{ json_encode(json_decode($request->drop_time)[$index]) }}'>{{ $date }}</option>
                                                                            @endforeach
                                                                        </optgroup>
                                                                    </select>
                                                                </div>
                                                        ...
                                                        @else
                                                            @if(\Illuminate\Support\Str::startsWith($request->drop_date_type, 'F'))
                                                                <div class="form-group">
                                                                    <div class="col-md-12">
                                                                        <label for="select2">Select Drop Date</label>
                                                                    </div>
                                                                    <div class="col-md-12">
                                                                        <select name="drop_booking_date" class="form-control" id="select2">
                                                                            <optgroup label="Preferred Drop Date">
                                                                                <option value="{{ $request->drop_prefer_date }}"
                                                                                        data-time="{{ $request->drop_prefer_time }}">{{ $request->drop_prefer_date }}</option>
                                                                            </optgroup>
                                                                            <optgroup label="Secondary Drop Date">
                                                                                @foreach(explode(',', $request->drop_date) as $index => $date)
                                                                                    <option value="{{ $date }}">{{ $date }}</option>
                                                            ...
                                                            @else
                                                                <label for="date"
                                                                       class="col-md-12 col-form-label">Drop Date</label>
                                                                <input class="form-control" type="text"
                                                                       name="drop_booking_date" value="{{ $request->drop_date }}"
                                                                       id="date" readonly>
                                                            @endif
                                                        @endif
                                                    </div>
                                                    <div class="col-md-6">
                                                        @if($request->drop_date_type == "FF")
                                                            <label for="dropTime"
                                                                   class="col-md-12 col-form-label">Drop Time</label>
                                                            <select name="drop_time" class="mySelect form-control" id="dropTime">
                                                                @foreach(json_decode($request->drop_time) as $myTime)
                                                                    <option value="{{ $myTime[0] }}">{{ $myTime[0] }}</option>
                                                                @endforeach
                                                            </select>
                                                        @else
                                                            @if(\Illuminate\Support\Str::endsWith($request->drop_date_type, 'F'))
                                                                <label for="dropTime"
                                                                       class="col-md-12 col-form-label">Drop Time</label>
                                                                <select name="drop_time" class="mySelect form-control" id="dropTime">
                                                                    @foreach(json_decode($request->drop_time) as $myTime)
                                                            ...
                                                            @else
                                                                <label for="date"
                                                                       class="col-md-12 col-form-label">Drop Time</label>
                                                                <input class="form-control" type="text"
                                                                       name="drop_time" value="{{ $request->drop_time }}"
                                                                       id="date" readonly>
                                                            @endif
                                                        @endif
                                                    </div>
