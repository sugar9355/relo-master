                ...
                <div class="row">
                    ...
                    <div class="form-body">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label class="control-label col-md-6">Junk Removal:</label>
                                    <div class="col-md-6">
                                        <p class="form-control-static"> {{ $request->junk_removal }} </p>
                                    </div>
                                </div>
                            </div>
                        </div>
...
@section('scripts')
    <script type="text/javascript">
		...
		function initMap() {
			...
			let source = new google.maps.LatLng("{{ $request->s_lat }}", "{{ $request->s_lng }}");
            ...
            @endforeach
				...
				source = new google.maps.LatLng("42.334900", "-71.056340");
			destination = new google.maps.LatLng("42.334900", "-71.056340");
			...
			let marker{{ 0 }} = new google.maps.Marker({
				map: map,
				icon: '/asset/img/marker-end.png',
				anchorPoint: new google.maps.Point(0, -29)
			});

			let markerPosition{{ 0 }} = new google.maps.LatLng("{{ $request->s_lat }}", "{{ $request->s_lng }}");
			marker{{ 0 }}.setPosition(markerPosition{{ 0 }});
			bounds.extend(marker{{ 0 }}.getPosition());

                    @foreach(json_decode($request->way_point_lats) as $index => $lat)
			let marker{{ $index + 1 }} = new google.maps.Marker({
					map: map,
					icon: '/asset/img/marker-end.png',
					anchorPoint: new google.maps.Point(0, -29)
				});
