...
<div class="contain-calendar">
    <div style="" class="calendar-title ">
        <i style="" class="far fa-calendar-alt "></i> Pickup Date
    </div>
    @foreach ($calender1 as $month => $value)
    @if($month == Intval(date('m')) || $month == (Intval(date('m')) + 1) || $month == (Intval(date('m')) + 2))

    <div id="month_{{$month}}_left" class="card hvr-shadow w-100 card-body  text-muted"
        ...
        <div class="row col-md-12 mb-1 mt-1">
            <div class="col-md-10"></div>
            <div class="col-md-1">
                @if($month == (Intval(date('m')) + 1) || $month == (Intval(date('m')) + 2))
                <button type="button" onclick="show_month_left('<','{{$month}}');"
                    ...
                    @foreach($week as $day)
                        ...
                        if(count($booking_dates) > 0) {
                            foreach ($booking_dates as $b_data) {
                                if ($b_data->type == 0 && $b_data->date == $date) {
                        ...
                        @endphp
                        @if($date > now())
                        <td id="td_pickup_{{$date}}" onclick="open_modal('{{$date}}', 0)"
                            class="cursor pickup_dates @if ($date == $pickup_date)shadow-date  @endif"
                        ...
                        @if($day[0] > 0)
                            ...
                            if (isset($charges['peak_factor'][strtotime($date)])) {
                                $d_charge = $charges['shuffle_price'][0] * $charges['peak_factor'][strtotime($date)]['pickup'] + $charges['pickup_items'] + ($charges['storage'] / 2);
                            }
                            @endphp
                            {{$day[0]}}
                            <span class="pickup_charge" style="display: none">{{number_format($d_charge, 2)}}</span>
                        @endif
                        </td>
                    @endforeach
                </tr>
...
<div class="modal fade " id="modalPoll-0" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-backdrop="false">
    <div class=" modal-dialog modal-full-height modal-right modal-notify modal-info drop" role="document">
        <div class="modal-content ">
            <!--Header-->
            <div class="modal-header bg-warning text-center">
                <span  id="date_0">Time</span>
                ...
                <div class="text-center">
                    ...
                    <div class="form-check col-8 mb-4 div-color1 m-auto mb-3">
                         <label class="form-check-label ml-2 col-12" for="radio-179">
                            Do you have any time constraints on the move out time?
                        </label>
                        {{--<label class="form-check-label ml-2 col-12" for="radio-179">
                            <button style="outline:none; border:none; " class="btn-sm btn-warning slideup" onclick="show_time(0)">Yes</button>
                            <button style="outline:none; border:none;  close" data-dismiss="modal" aria-label="Close"
                            class="btn-sm btn-warning" onclick="update_date_only(0)">No</button>
                        </label> --}}
                        ...
                        <hr>
                        <label class="form-check-label ml-2 col-12" id="text_flexible_0" style="font-size: {{$font_info_flexible->font_size}}px;color: {{$font_info_flexible->color}}">
                            {{$font_info_flexible->name}}
                        </label>
                        <label class="form-check-label ml-2 col-12" id="text_unflexible_0" style="font-size: {{$font_info_unflexible->font_size}}px;color: {{$font_info_unflexible->color}};display: none">
                            {{$font_info_unflexible->name}}
                        </label>
                    </div>
                    <div id="panel" class="form-check mb-4 div-color1 col-8 m-auto pt-3" style="display: none;">
                        <div id="time-range-0">
                            <p>Time Range: <span class="slider-time-0 font-weight-bold">06:00 AM</span> - <span class="slider-time2-0 font-weight-bold">09:00 PM</span>
                            </p>
                            <div class="sliders_step1">
                                <div id="slider-range-0"></div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div style="border-top: 1px solid #dee2e6; " class="modal-footer justify-content-center">
                <a type="button" class="btn btn-outline-success dvvv drop-btn" onclick="update_date_time(0)">Continue</a>
            </div>
        </div>
    </div>
</div>

<input type="hidden" id="chosen_date_0" />
<input type="hidden" id="chosen_start_time_0" />
<input type="hidden" id="chosen_end_time_0" />