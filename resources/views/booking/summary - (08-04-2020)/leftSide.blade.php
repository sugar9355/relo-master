...
<hr>
	...
	<div class="bg-light hvr-shadow hvr-icon-bob p-2 rounded mb-3">
	<div class="custom-control custom-radio">
	<input type="radio" data-toggle="modal" data-target="#insurance-pop" class="custom-control-input" name="insurance" id="insurance{{$insurance->id}}" @if($booking->insurance == $insurance->amount) checked @endif >
	<label class="custom-control-label w-100 text-{{$insurance->color}}" for="insurance{{$insurance->id}}">{{$insurance->name}} (${{$insurance->amount}}) <i class="fas fa-info-circle text-muted hvr-icon"></i></label>
	</div>
	</div>
	@endforeach
	
	@include('booking.summary.modal_insurance')
